    ...
    class Program
    {
        static void Main(string[] args)
        {
...

            {
                ...
                //TODO 1
                List<string> strings = new List<string> {"qewt", "5hfkf", "fjfkh7", "jfnfj49j"};
                int sum = strings.Select(x => x.Length).Sum();
                // or int sum = strings.Sum( x=> x.Length);
            }

            {
                ...
                //TODO 2
                List<string> strings = new List<string> {"qewt", "5hfkf", "fjfkh7"};
                //string res = new string(strings.Select(s => s[0]).ToArray());
                string shortName = new string(strings.Where(s => !string.IsNullOrEmpty(s))
...

            {
                ...
                //TODO 3
                int k = 4;
                List<string> strings = new List<string> {"QGTE", "5GHJ", "YHHJ7", "5646", "GT45", "Agh3"};

                var stringsQuery =
                    from str in strings
                    where (str.Length == k) && Char.IsDigit(str[str.Length - 1])
                    select str;
                stringsQuery = stringsQuery.OrderBy(x => x[x.Length - 1]);
                //stringsQuery.ToList().ForEach(s => Console.WriteLine(s));
                //strings.Where(s => s.Length == k && Char.IsDigit(s[s.Length - 1])).ToList().ForEach(s => Console.WriteLine(s));
            }

            {
                ...
                //TODO 4
                int D = 2, K = 1;
                List<int> A = new List<int> {3, 70, 5, 5, 1, 6, 3};
                var res = A.TakeWhile(x => x > D).Skip(K).Distinct().OrderBy(x => x);
...

            {
                ...
                //TODO 5
                List<string> strings = new List<string> {"QGTE", "5GHJ", "YHHJ7", "5646", "GT45", "Agh3"};
                string res = new string(strings.Select(x =>
                {
                    if (x.Length % 2 != 0)
                        return x[0];
                    else
...

            {
                ...
                int K1 = 3;
                int K2 = 4;
                List<int> A = new List<int> {3, 5, 6, 3, 6};
                List<int> B = new List<int> {34, 532, 625, 32, 62};
                var res1 = A.FindAll(x => x > K1);
                var res2 = B.FindAll(x => x < K2);
                var res = res1.Concat(res2).OrderBy(x => x).ToArray();
                //res.ToList().ForEach(x => Console.Write(x + " "));
            }
            {
...
//              TODO 7
                List<string> A = new List<string> {"HH", "FFF344", "YYY7"};
                List<string> B = new List<string> {"HHH6", "VEG5", "YY"};

                var innerJoinQuery =
                    from a in A
                    join b in B on a.Length equals b.Length
                    select a[0].ToString() + a[1].ToString() + ": " + b[0].ToString() + b[1].ToString();

                foreach (string item in innerJoinQuery)
...

            {
...

                {
                    ...
                    //TODO 8
                    List<Enrollee> list = new List<Enrollee>()
                    {
                        new Enrollee {Surname = "Joke", School = 3, YearOfEnroll = 2013},
                        new Enrollee {Surname = "Plid", School = 2, YearOfEnroll = 2014},
                        new Enrollee {Surname = "Urin", School = 4, YearOfEnroll = 2014}
                    };

                    List<Res> result = list
                        .GroupBy(l => l.YearOfEnroll)
                        .Select(c1 => new Res
                        {
                            SchoolCount = c1.Count(),
                            Year = c1.First().YearOfEnroll
                        }).OrderBy(x => x.SchoolCount).OrderBy(x => x.Year).ToList();

//                    foreach (var l in result)
//                    {
//                        Console.WriteLine("Year:" + l.Year + "\tAmount: : " + l.SchoolCount);
//                    }
                }
            }
        }
